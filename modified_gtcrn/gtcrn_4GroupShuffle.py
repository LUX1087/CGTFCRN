...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.en_convs = nn.ModuleList([
            ...
            ConvBlock(16, 8, (1,5), stride=(1,2), padding=(0,2), groups=2, use_deconv=False, is_last=False),
            GTConvBlock(16, 8, (3,3), stride=(1,1), padding=(0,1), dilation=(1,1), use_deconv=False),
            GTConvBlock(16, 8, (3,3), stride=(1,1), padding=(0,1), dilation=(2,1), use_deconv=False),
            GTConvBlock(16, 8, (3,3), stride=(1,1), padding=(0,1), dilation=(3,1), use_deconv=False),
            GTConvBlock(16, 8, (3, 3), stride=(1, 1), padding=(0, 1), dilation=(5, 1), use_deconv=False),
        ])
    def shuffle(self, x):
        """x1, x2: (B,C,T,F)"""
        x = torch.stack(torch.chunk(x, chunks=2, dim=1), dim=1).transpose(1, 2).contiguous()  # (B,C,2,T,F)
        x = rearrange(x, 'b c g t f -> b (c g) t f')  # (B,2C,T,F)
        return x
    def forward(self, x):
        en_outs = []
        for i in range(len(self.en_convs)):
            x = self.en_convs[i](x)
            if bAddShuffle and i == len(self.en_convs) - 1:
                x = self.shuffle(x)
            en_outs.append(x)
        return x, en_outs
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.de_convs = nn.ModuleList([
            GTConvBlock(16, 8, (3, 3), stride=(1, 1), padding=(2 * 5, 1), dilation=(5, 1), use_deconv=True),
            GTConvBlock(16, 8, (3,3), stride=(1,1), padding=(2*5,1), dilation=(3,1), use_deconv=True),
            GTConvBlock(16, 8, (3,3), stride=(1,1), padding=(2*2,1), dilation=(2,1), use_deconv=True),
            GTConvBlock(16, 8, (3,3), stride=(1,1), padding=(2*1,1), dilation=(1,1), use_deconv=True),
            ConvBlock(16, 8, (1,5), stride=(1,2), padding=(0,2), groups=2, use_deconv=True, is_last=False),
    ...
    def forward(self, x, en_outs):
        ...
        for i in range(N_layers):
            ...
            if bAddShuffle and i == len(self.de_convs) - 1:
                x = self.shuffle(x)
        return x
